...
### III. Python as Primary Implementation Language

All implementation code in this repo is Python. Tooling and configuration specs (pre-commit hooks, CI workflows, coverage config) produce YAML/TOML/config files — not Python code — and are explicitly scoped as such in their spec. Mixed-language specs are not permitted; each spec targets one output type.
...
Every spec MUST define at least one acceptance scenario that can be verified by running a command (e.g., `pytest`, `pre-commit run --all-files`, `python -m <module>`). Specs that produce only prose, documentation, or unrunnable artifacts are out of scope for this harness.
